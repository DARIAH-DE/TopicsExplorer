...
def upload_file():
        ...
        for p in pos_tags:  # collect only the specified parts-of-speech
            ...
            if doc_split:  # size according to paragraph id
                ...
                for para_id, para in doc:
                    ...
                    doc_labels.append(''.join([file_label, " #",  \
                    str(para_id)])) # use name + doc id as plot label
            ...
            with open(swpath + ".txt", 'w', encoding = "utf-8") as text:
                text.write(" ".join(word for word in \
                doc['Lemma'].values.astype(str) if word not in stopwords))
    ...
    open(mastercorpus, encoding="utf-8"))
    ...
    with open(os.path.join(os.path.join(os.getcwd(), "out"),''.join(["corpus_doclabels.txt"])), "w", encoding="utf-8") as f:
        for item in doc_labels: f.write(item+"\n")

    with open(os.path.join(os.path.join(os.getcwd(), "out"), ''.join(["corpus_topics.txt"])), "w", encoding="utf-8") as f:
        for item, i in zip(topics, enumerate(topics)):
