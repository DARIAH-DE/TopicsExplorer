...
#    for file in corpus:
#        with open(file, 'r', encoding='utf-8') as f:
#            value = Counter(f.read().split())
#    
#        hapax = set([hapx for hapx, value if value == 1])
#
#    return hapax
#    
...
def filter_POS_tags(df, pos_tags):
...
class Visualization:
    ...
    def make_heatmap(self):
        ...
        heatmap = plt.figure(figsize=(20,20))    # if many items, enlarge figure
        heatmap.pcolor(doc_topic, norm=None, cmap='Reds')
        heatmap.yticks(np.arange(doc_topic.shape[0])+1.0, doc_labels)
        heatmap.xticks(np.arange(doc_topic.shape[1])+0.5, topic_labels, rotation='90')
        heatmap.gca().invert_yaxis()
        heatmap.colorbar(cmap='Reds')
